...
var PERMANENT_URL_PREFIX = 'media/';
...
function cleanupPreWhitespace() {
  var els = document.getElementsByTagName('pre');
  for (var i = 0, el; el = els[i]; i++) {
    if (el.classList.contains("no-strip-whitespace"))
      continue;
    // strip off leading and trailing lines
    var html = el.innerHTML;
    // find the smallest amount of leading whitespace
    var lines = html.split(/[\n\r]/);
    var toStrip = Number.POSITIVE_INFINITY;
    for (var j = 0; j < lines.length; j += 1) {
      ...
      if (firstNonWhitespace < 0)
        continue;
      toStrip = Math.min(firstNonWhitespace - 1, toStrip);
    }
...
function addPrettify() {
  var els = document.getElementsByClassName("prettify");
  for (var i = 0, el; el = els[i]; i++) {
